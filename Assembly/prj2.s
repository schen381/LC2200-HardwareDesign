...
ti_inthandler:
    ...
    beq $zero, $a1, imin        ! If a1 = 0, need to increment minutes
    sw $s1, 0x00($s0)           ! Otherwise, push seconds back to memory
    beq $zero, $zero, fin       ! Unconditionally branch to fin state
imin: sw $zero, 0x00($s0)       ! First, push seconds=0 into memory
    ...
    beq $zero, $zero, fin       ! Unconditionally branch to finish state
